...
## Table of Contents
- [Process](#process)
- [Step 1: Build the page inventory](#step-1-build-the-page-inventory)
- [Step 2: Run all checks](#step-2-run-all-checks)
- [Step 3: Write the lint report](#step-3-write-the-lint-report)
- [Step 4: Offer concrete fixes](#step-4-offer-concrete-fixes)
- [Step 5: Append to 06 Wiki/log.md](#step-5-append-to-wikilogmd)
...
Use `obsidian search query="path:06 Wiki/pages"` to get the list of pages. Build a map of:
- All existing slugs (filenames without `.md`)
- All `[[slug]]` references found in any page (use `obsidian backlinks` or a global grep)
- All `sources` listed in frontmatter (use `obsidian property:get` if possible)

### Step 2: Run all checks

**🔴 Errors (must fix)**

- **Broken links** — `[[slug]]` references where no corresponding `06 Wiki/pages/<slug>.md` exists
- **Missing frontmatter** — pages without required `title`, `tags`, `sources`, or `updated` fields (use `obsidian read` to check frontmatter)
...
Write `06 Wiki/pages/lint-<today>.md` as an Obsidian note (do not ask permission — always write this). Ensure it uses Obsidian callouts for the report sections and [[wikilinks]] for page references.
...
Add the lint report to `06 Wiki/index.md` under a Maintenance category (create it if it doesn't exist).

### Step 4: Offer concrete fixes
...
- **Orphan page tags:** "Add `status: orphan` to frontmatter of orphan pages (using `obsidian property:set`)?"
- **Missing frontmatter:** "Add missing frontmatter fields with placeholder values?"

Show the exact diff for each change before writing. Apply only after confirmation. Use `obsidian` commands where possible to make edits.

### Step 5: Append to `06 Wiki/log.md`
