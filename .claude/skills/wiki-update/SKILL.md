...
- **New information** — read `06 Wiki/index.md` to find affected pages, then read those pages
...
After identifying the primary pages to update, grep for `[[slug]]` references to those pages across all of `06 Wiki/pages/`. For each page that links to an updated page:
...
- **Skipping the check** — An update that contradicts a page's content while leaving pages that link to it unchanged creates silent inconsistency.
